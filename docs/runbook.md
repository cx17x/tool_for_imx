...
```bash
python3 object_detection.py --target-class airplane
```
...
```bash
python3 object_detection.py --threshold 0.10
```

Изменить IoU для postprocess:

```bash
python3 object_detection.py --iou 0.45
```
...
```bash
python3 object_detection.py --main-width 640 --main-height 640 --mjpeg --mjpeg-host 0.0.0.0 --mjpeg-port 8081 --mjpeg-quality 90 --no-preview
```
